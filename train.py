    ...
    if(sch_state < 1):
        ...
        for i, factor in enumerate(factors):
            if(i < sch_state):
                ...
                break
        optimizer.param_groups[0]['lr'] = learning_rate
    print(f"Preloaded optimizer from \"{preload_opt_path}\" with learning rate {learning_rate}")
...
small_test_end = int(((len(trainloader)*(max_time/test_max_time))+len(testloader))*0.01)

# Do not run small test ff learning rate will not be reduced by more than scheduler EPS
step_sch = (optimizer.param_groups[0]['lr']*(1-scheduler.factor) > scheduler.eps)
...
for epoch in range(run["hparams", "num_epochs"]):
    ...
    for i, (comp_wav, fileinfo) in pbar:
        ...
        if(batch_size > 1):
            ...
            if(uncomp_wav.shape[2] != comp_wav.shape[2]):
                print(f"{fileinfo[0][0]} mismatched size, input is {comp_wav.shape[2]} and label is {uncomp_wav.shape[2]}")
        ...
        # Adapted from https://github.com/pseeth/autoclip/blob/master/autoclip.py, removes Pytorch Ignite dependency
        # Clip gradients
        tot_norm = 0
        ...
        if(save_partial_epoch):
            ...
            #if(i%test_point == 0 and i < last_test_point):
                if(step_sch):
                    # Small testing phase
                    model.eval()
                    with torch.no_grad():
                        ...
                        avg_test_loss = 0
                        avg_test_acc = 0
                        test_pbar = tqdm(enumerate(testloader), total=small_test_end+1, desc=f"Small Test Epoch {scheduler.last_epoch+1}")
                        for i, (comp_wav, fileinfo) in test_pbar:
                            # Stop small test after small_test_end batches
                            if(i > small_test_end):
                                break

                            # Don't check index in small test if in list of outliers
                            if(not i in exclude_small_test):
                                ...
                                # Don't include outliers in small test
                                if(scheduler.last_epoch < 1 and loss.item() > exclude_thres):
                                    print(f"{fileinfo} with {loss.item():.4f} loss to be excluded from small test")
                                    small_test_end = small_test_end + 1
                    ...
                    avg_test_loss = tot_test_loss/(small_test_end+1-len(exclude_small_test))
                    avg_test_acc = tot_test_acc/(small_test_end+1-len(exclude_small_test))

                    # Send average training loss to scheduler
                    scheduler.step(avg_test_loss)

                    # Log average test loss and accuracy to aim and terminal
                    run.track(avg_test_loss, name='avg_loss', step=scheduler.last_epoch, epoch=epoch+1, context={ "subset":"small_test" })
                    run.track(avg_test_acc, name='avg_acc', step=scheduler.last_epoch, epoch=epoch+1, context={ "subset":"small_test" })
                    print(f"Small Test Epoch {scheduler.last_epoch}: Avg_Loss={avg_test_loss:.4f}, Avg_Acc={avg_test_acc:.4f}")
    ...
    with torch.no_grad():
        ...
        for i, (comp_wav, fileinfo) in pbar:
            ...
            if(i == small_test_end and save_partial_epoch):
                if(step_sch):
                    ...
                    avg_stest_loss = tot_stest_loss/(small_test_end+1-len(exclude_small_test))
                    # Send average small test loss to scheduler
                    scheduler.step(avg_stest_loss)
                    # Print average small test loss to terminal
                    print(f"Small Test Epoch {scheduler.last_epoch}: Avg_Loss={avg_stest_loss:.4f}")
                    # Check if learning rate was changed
                    if(optimizer.param_groups[0]['lr'] < learning_rate):
                        # Save current learning rate
                        learning_rate = optimizer.param_groups[0]['lr']
        ...
        run.track(avg_loss, name='avg_loss', step=epoch+1, epoch=epoch+1, context={ "subset":"test" })
        run.track(avg_acc, name='avg_acc', step=epoch+1, epoch=epoch+1, context={ "subset":"test" })
